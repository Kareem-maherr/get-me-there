<template>
  <div id="Home" class="flex flex-col h-screen">
	<div class="m-3 mb-auto">
		<div class="w-full background-color h-14 rounded-full flex items-center p-3">
			<MagnifyIcon :size="40" />
			<div class="ml-3 text-xs font-semibold text-gray-700">
				Enter Pickup Point
			</div>
		</div>
		<div class="grid grid-cols-2 gap-3 my-3">
			<ServiceSelectLarge 
				text="Ride"
				imageWidth= "74"
				image="ride"
				promo="true"
			/>
			<ServiceSelectLarge 
				text="Food"
				imageWidth= "72"
				image="food"
			/>
		</div>
		<div class="grid grid-cols-4 gap-3 my-3">
			<ServiceSelectSmall
				text="2 Wheels"
				image="2wheels" />
			<ServiceSelectSmall
				text="Transit"
				image="transit" />
			<ServiceSelectSmall
				text="Travel"
				image="travel" />
			<ServiceSelectSmall
				text="Reserve"
				image="reserve" />
			
		</div>
		<div class="grid grid-cols-2 gap-3 my-3" :wrap-around="true">
			<PromosComp
				text="Upgrade to GetMeThere+"
				imageWidth="74"
				image="ride" />
			<PromosComp
				text="Book A Lemo"
				imageWidth="74"
				image="ride" />
		</div>
	</div>
	<footer class="w-full z-50 flex justify-around p-1 mb-2 border-t-2 border-t-gray-200">
		<div class="grid place-items-center">
			<HomeIcon :size="30" fillColor="#1f1e1e" />
			<div class="w-full -mt-1 text-xs text-center text-gray-800 font-semibold">
				Home
			</div>
		</div>
		<div class="grid place-items-center">
			<AccountIcon :size="30" fillColor="#1f1e1e" />
			<div class="w-full -mt-1 text-xs text-center text-gray-800 font-semibold">
				Account
			</div>
		</div>
	</footer>
  </div>
</template>

<script setup>
import ServiceSelectLarge from '../components/ServiceSelectLarge.vue';
import ServiceSelectSmall from '../components/ServiceSelectSmall.vue';
import PromosComp from '../components/PromosComp.vue';

import MagnifyIcon from 'vue-material-design-icons/Magnify.vue';
import HomeIcon from 'vue-material-design-icons/Home.vue';
import AccountIcon from 'vue-material-design-icons/Account.vue'

</script>

<style lang="scss" scoped>
	#Home {

		.background-color{
			background-color: rgb(237, 237, 237);
		}

		.background-height{
			height: 100px;
		}
	}


</style>